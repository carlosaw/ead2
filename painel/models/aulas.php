...
class Aulas extends model {
	...
	public function addAula($id_curso, $id_modulo, $nome, $tipo) {
		$sql = "SELECT ordem FROM aulas WHERE id_modulo = '$id_modulo' ORDER BY ordem DESC LIMIT 1";//Pega a ultima ordem daquele curso
		$sql = $this->db->query($sql);
		$ordem = 1;
		if($sql->rowCount() > 0) {
			$sql = $sql->fetch();
			$ordem = intval($sql['ordem']);//Vem string transf em numero
			$ordem++;
		}
		$sql = "INSERT INTO aulas SET id_modulo = '$id_modulo', id_curso = '$id_curso', ordem = '$ordem', tipo = '$tipo'";
			$this->db->query($sql);//Adiciona a aula aqui.
